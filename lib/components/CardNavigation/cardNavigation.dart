...
const Color _kAppBackgroundColor = Colors.blue;
  ...
  void performLayout(Size size) {
    ...
    for (int index = 0; index < cardCount; index++) {
      ...
      final Rect rowCardRect =
          new Rect.fromLTWH(rowCardX, 0.0, rowCardWidth, rowCardHeight);
      final Rect cardRect =
      ...
      final double centeredRowIndicatorX = rowIndicatorX +
          (paddedSectionIndicatorWidth - indicatorSize.width) / 2.0;
...
class AnimateTabNavigation extends StatefulWidget {
  const AnimateTabNavigation({Key key, @required this.sectionList})
      : super(key: key);
...
class _AnimateTabNavigationState extends State<AnimateTabNavigation> {
  ...
  bool _handleScrollNotification(
      ScrollNotification notification, double minScrollOffset) {
    if (notification is ScrollUpdateNotification) {
      _expanded = _scrollController.position.pixels >= minScrollOffset;
      final ScrollPhysics physics = _expanded
  ...
  void _maybeScroll(double minScrollOffset, int pageIndex, double xOffset,
      int totalPageCount) {
  ...
  Iterable<Widget> _allHeadingItems(
      double maxHeight, double minHeight, double minScrollOffset) {
      ...
      sectionCards.add(new LayoutId(
        ...
        child: new GestureDetector(
            ...
            onTapUp: (TapUpDetails details) {
              ...
              setState(() {
                _maybeScroll(
                    minScrollOffset, index, xOffset, sectionList.length);
              });
            }),
      ));
    }
          ...
          new NotificationListener<ScrollNotification>(
            onNotification: (ScrollNotification notification) {
              return _handleScrollNotification(
                  notification, appBarMinScrollOffset);
            },
            child: new CustomScrollView(
              controller: _scrollController,
              physics: new _SnappingScrollPhysics(
                  minScrollOffset: appBarMinScrollOffset),
              slivers: <Widget>[
                    ...
                    child: new NotificationListener<ScrollNotification>(
                      ...
                      child: new PageView(
                        ...
                        children: _allHeadingItems(
                            appBarMaxHeight,
                            _kAppBarMinHeight + statusBarHeight,
                            appBarMinScrollOffset),
                      ),
                    ),
                  ),
                ),
